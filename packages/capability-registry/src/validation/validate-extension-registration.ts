...
import {
  ...
  EXTENSION_REGISTRATION_SEAM_TYPES,
  normalizeOpaqueExtensionReference,
  normalizeOpaqueExtensionReferences,
  type CapabilityRegistration,
  type ExecutionAdapterRegistration,
  type ExtensionAdapterLaunchMode,
  type ExtensionCapabilityRegistrationDetails,
  type ExtensionRegistration,
  type ExtensionRegistrationDiagnostic,
  type ExtensionRegistrationLocalRefs,
  type ExtensionRegistrationMetadata,
  type ExtensionRegistrationSeamType,
  type ExtensionSkillPackRegistrationDetails,
  type SkillPackRegistration,
} from "../../../contracts/src/extension/extension-registration";

const LAUNCH_MODES: ExtensionAdapterLaunchMode[] = [
  "foreground_only",
  "background_only",
  "foreground_or_background",
];

const FORBIDDEN_FIELD_RULES = new Map<string, { code: string; message: string }>([
  ["marketplace", {
...
export interface ValidateExtensionRegistrationOptions {
  baseDir?: string;
  sourcePath?: string;
}

export interface ExtensionRegistrationValidationResult {
  ok: boolean;
  diagnostics: ExtensionRegistrationDiagnostic[];
  registration: ExtensionRegistration | null;
  resolvedLocalRefs: ResolvedExtensionRegistrationLocalRefs | null;
}

export function validateExtensionRegistration(
  value: unknown,
  options: ValidateExtensionRegistrationOptions = {},
): ExtensionRegistrationValidationResult {
  ...
  if (!isRecord(value)) {
    pushDiagnostic(
      ...
      "invalid_type",
      "$",
      "Le manifeste doit etre un objet JSON racine.",
    );
  ...
  if (
    schemaVersion !== null
    && schemaVersion !== EXTENSION_REGISTRATION_SCHEMA_VERSION
  ) {
    pushDiagnostic(
      diagnostics,
      "unsupported_schema_version",
      "schemaVersion",
      `La version de schema doit valoir \`${EXTENSION_REGISTRATION_SCHEMA_VERSION}\`.`,
    );
  }
  ...
  let registration: ExtensionRegistration | null = null;

  if (seamType === "execution_adapter") {
    registration = readExecutionAdapterRegistration(
      value,
      {
        schemaVersion: EXTENSION_REGISTRATION_SCHEMA_VERSION,
        seamType,
  ...
  if (seamType === "capability") {
    registration = readCapabilityRegistration(
      value,
      {
        schemaVersion: EXTENSION_REGISTRATION_SCHEMA_VERSION,
        seamType,
  ...
  if (seamType === "skill_pack") {
    registration = readSkillPackRegistration(
      value,
      {
        schemaVersion: EXTENSION_REGISTRATION_SCHEMA_VERSION,
        seamType,
  ...
  if (registration === null && diagnostics.length === 0) {
    pushDiagnostic(
      ...
      "internal_error",
      "seamType",
      `Le type de seam \`${seamType}\` est reconnu mais aucune branche de traitement n'est implementee.`,
    );
  }
...
function resolveBaseDir(options: ValidateExtensionRegistrationOptions): string | undefined {
  if (options.baseDir) {
    return path.resolve(options.baseDir);
  }

  if (options.sourcePath) {
    return path.dirname(path.resolve(options.sourcePath));
  }

  return undefined;
}

function readExecutionAdapterRegistration(
  value: Record<string, unknown>,
  baseRegistration: Omit<ExecutionAdapterRegistration, "executionAdapter">,
  diagnostics: ExtensionRegistrationDiagnostic[],
): ExecutionAdapterRegistration | null {
  ensureExclusiveSeamBlock(
    value,
    "execution_adapter",
    diagnostics,
  );
...
function readCapabilityRegistration(
  value: Record<string, unknown>,
  baseRegistration: Omit<CapabilityRegistration, "capability">,
  diagnostics: ExtensionRegistrationDiagnostic[],
...
function readSkillPackRegistration(
  value: Record<string, unknown>,
  baseRegistration: Omit<SkillPackRegistration, "skillPack">,
  diagnostics: ExtensionRegistrationDiagnostic[],
): SkillPackRegistration | null {
  ensureExclusiveSeamBlock(
    value,
    "skill_pack",
    diagnostics,
...
function ensureExclusiveSeamBlock(
  value: Record<string, unknown>,
  expectedSeamType: ExtensionRegistrationSeamType,
  diagnostics: ExtensionRegistrationDiagnostic[],
): void {
  const seamBlockByType: Record<ExtensionRegistrationSeamType, string> = {
  ...
  for (const [seamType, blockName] of Object.entries(seamBlockByType)) {
    ...
    if (blockName === expectedBlock && blockValue === undefined) {
      pushDiagnostic(
        ...
        blockName,
        `Le bloc \`${blockName}\` est requis pour \`${seamType}\`.`,
      );
      continue;
    }
    ...
    if (blockName !== expectedBlock && blockValue !== undefined) {
      pushDiagnostic(
        diagnostics,
        "unexpected_seam_block",
        blockName,
        `Le bloc \`${blockName}\` ne peut pas coexister avec \`${expectedBlock}\` dans une declaration V1.`,
      );
    }
  }
}

function readExecutionAdapterDetails(
  value: Record<string, unknown>,
  diagnostics: ExtensionRegistrationDiagnostic[],
): ExecutionAdapterRegistration["executionAdapter"] | null {
  ...
  if (launchMode === "foreground_only" && supportsBackground) {
    pushDiagnostic(
      diagnostics,
      "invalid_field_value",
      "executionAdapter.supportsBackground",
      "Un adaptateur `foreground_only` ne peut pas declarer `supportsBackground=true`.",
    );
  }

  if (
    (launchMode === "background_only" || launchMode === "foreground_or_background")
    && !supportsBackground
  ) {
    pushDiagnostic(
      diagnostics,
      "invalid_field_value",
      "executionAdapter.supportsBackground",
      "Un adaptateur pouvant tourner en background doit declarer `supportsBackground=true`.",
    );
  }

  return {
    adapterRuntimeId: normalizeOpaqueExtensionReference(adapterRuntimeId),
    launchMode,
    supportsBackground,
    requiredEnvNames,
  };
...
function readCapabilityDetails(
  value: Record<string, unknown>,
  localRefs: ExtensionRegistrationLocalRefs,
  diagnostics: ExtensionRegistrationDiagnostic[],
): ExtensionCapabilityRegistrationDetails | null {
  ...
  const registration: ExtensionCapabilityRegistrationDetails = {
    capabilityId: normalizeOpaqueExtensionReference(capabilityId),
    provider,
    approvalSensitive,
    requiredEnvNames,
  };
  ...
  if (provider === "local" && !localRefs.entrypoint) {
    pushDiagnostic(
      diagnostics,
      "missing_required_field",
      "localRefs.entrypoint",
      "Une capability locale doit declarer `localRefs.entrypoint`.",
    );
  }
  ...
  if (provider === "mcp") {
    ...
    if (localRefs.entrypoint) {
      pushDiagnostic(
        ...
        "unexpected_field",
        "localRefs.entrypoint",
        "Une capability MCP-backed ne doit pas declarer d'entree locale executable.",
      );
    }

    registration.mcpServerName = normalizeOpaqueExtensionReference(mcpServerName);
    registration.mcpToolName = normalizeOpaqueExtensionReference(mcpToolName);
  }

  return registration;
}

function readSkillPackDetails(
  value: Record<string, unknown>,
  diagnostics: ExtensionRegistrationDiagnostic[],
): ExtensionSkillPackRegistrationDetails | null {
  ...
  return {
    packRef: normalizeOpaqueExtensionReference(packRef),
  };
}

function readSeamType(
  value: Record<string, unknown>,
  diagnostics: ExtensionRegistrationDiagnostic[],
): ExtensionRegistrationSeamType | null {
  ...
  if (EXTENSION_REGISTRATION_SEAM_TYPES.includes(seamType as ExtensionRegistrationSeamType)) {
    return seamType as ExtensionRegistrationSeamType;
  }
  ...
  pushDiagnostic(
    ...
    "invalid_field_value",
    "seamType",
    "Le `seamType` doit valoir `execution_adapter`, `capability` ou `skill_pack`.",
  );
...
function readLaunchMode(
  value: Record<string, unknown>,
  diagnostics: ExtensionRegistrationDiagnostic[],
): ExtensionAdapterLaunchMode | null {
  ...
  if (LAUNCH_MODES.includes(launchMode as ExtensionAdapterLaunchMode)) {
    return launchMode as ExtensionAdapterLaunchMode;
  }

  pushDiagnostic(
    diagnostics,
    "invalid_field_value",
    "executionAdapter.launchMode",
    "Le `launchMode` doit valoir `foreground_only`, `background_only` ou `foreground_or_background`.",
  );

  return null;
}

function readCapabilityProvider(
  value: Record<string, unknown>,
  diagnostics: ExtensionRegistrationDiagnostic[],
): "local" | "mcp" | null {
  ...
  if (provider !== null) {
    pushDiagnostic(
      ...
      "invalid_field_value",
      "capability.provider",
      "Le provider d'une capability doit valoir `local` ou `mcp`.",
    );
  }
...
function readMetadata(
  value: Record<string, unknown>,
  diagnostics: ExtensionRegistrationDiagnostic[],
): ExtensionRegistrationMetadata | null {
...
function readLocalRefs(
  ...
  normalized: ExtensionRegistrationLocalRefs;
  resolved: ResolvedExtensionRegistrationLocalRefs;
} | null {
...
function readVersion(
  ...
  if (!SEMVER_PATTERN.test(version)) {
    pushDiagnostic(
      ...
      "invalid_field_value",
      "version",
      "La version doit suivre un format semver simple, par exemple `0.1.0`.",
    );
    return null;
  }
  ...
  for (const [index, envName] of values.entries()) {
    if (!ENV_NAME_PATTERN.test(envName)) {
      pushDiagnostic(
        ...
        "Les variables d'environnement doivent etre declarees par nom, en MAJUSCULES et sans valeur inline.",
      );
    }
  }
...
function readStringList(
  ...
  if (!Array.isArray(rawValue)) {
    pushDiagnostic(
      diagnostics,
      rawValue === undefined ? "missing_required_field" : "invalid_type",
      fieldPath,
      `Le champ \`${fieldPath}\` doit etre un tableau de chaines.`,
    );
    return null;
  }
  ...
  for (const [index, item] of rawValue.entries()) {
    if (typeof item !== "string") {
      pushDiagnostic(
        ...
        "invalid_type",
        `${fieldPath}[${index}]`,
        "Chaque entree doit etre une chaine non vide.",
      );
      continue;
    }
  ...
  return normalizeOpaqueExtensionReferences(stringValues);
}
...
function readRequiredBoolean(
  ...
  if (typeof rawValue !== "boolean") {
    pushDiagnostic(
      ...
      rawValue === undefined ? "missing_required_field" : "invalid_type",
      fieldPath,
      `Le champ \`${fieldPath}\` doit etre un booleen.`,
    );
    return null;
  }
...
function readRequiredString(
  ...
  if (typeof rawValue !== "string") {
    pushDiagnostic(
      ...
      rawValue === undefined ? "missing_required_field" : "invalid_type",
      fieldPath,
      `Le champ \`${fieldPath}\` doit etre une chaine non vide.`,
    );
    return null;
  }
...
function readRequiredRecord(
  ...
  if (!isRecord(rawValue)) {
    pushDiagnostic(
      ...
      rawValue === undefined ? "missing_required_field" : "invalid_type",
      key,
      `Le champ \`${key}\` doit etre un objet.`,
    );
    return null;
  }
...
function readLocalRef(
  ...
  if (rawValue === undefined) {
    if (options.required) {
      pushDiagnostic(
        ...
        "missing_required_field",
        fieldPath,
        `Le champ \`${fieldPath}\` est obligatoire.`,
      );
    }
  ...
  if (typeof rawValue !== "string") {
    pushDiagnostic(
      ...
      "invalid_type",
      fieldPath,
      `Le champ \`${fieldPath}\` doit etre une ref locale sous forme de chaine.`,
    );
    return null;
  }

  const relativePath = normalizeOpaqueExtensionReference(rawValue);
  ...
  if (!relativePath) {
    pushDiagnostic(
      ...
      "invalid_field_value",
      fieldPath,
      `Le champ \`${fieldPath}\` doit etre une ref locale non vide.`,
    );
    return null;
  }
  ...
  if (isRemoteReference(relativePath) || isAbsoluteReference(relativePath)) {
    pushDiagnostic(
      ...
      "non_local_ref",
      fieldPath,
      "Les refs du contrat V1 doivent rester locales et relatives au manifeste.",
    );
    return null;
  }
  ...
  if (containsForbiddenLocalRef(relativePath)) {
    pushDiagnostic(
      ...
      "forbidden_local_ref",
      fieldPath,
      "Le contrat V1 refuse `.codex/config.toml` et `.codex-plugin/plugin.json`.",
    );
    return null;
  }
...
function readLocalRefList(
  ...
  if (!Array.isArray(rawValue)) {
    pushDiagnostic(
      ...
      rawValue === undefined ? "missing_required_field" : "invalid_type",
      fieldPath,
      `Le champ \`${fieldPath}\` doit etre un tableau de refs locales.`,
    );
    return null;
  }

  const results: Array<{ relativePath: string; resolvedPath: string }> = [];
  ...
  for (const [index, rawItem] of rawValue.entries()) {
    if (typeof rawItem !== "string") {
      pushDiagnostic(
        ...
        "invalid_type",
        `${fieldPath}[${index}]`,
        "Chaque ref locale doit etre une chaine.",
      );
      continue;
    }

    const relativePath = normalizeOpaqueExtensionReference(rawItem);
...
function validateResolvedLocalRef(
  ...
  try {
    ...
    pushDiagnostic(
      ...
      "missing_local_ref",
      fieldPath,
      `La ref locale resolue n'existe pas: ${resolvedPath}`,
    );
    return;
  }
  ...
  if (expectedType === "file" && !stats.isFile()) {
    pushDiagnostic(
      diagnostics,
      "invalid_local_ref_type",
      fieldPath,
      "La ref locale attendue doit pointer vers un fichier.",
    );
  }
  ...
  if (expectedType === "directory" && !stats.isDirectory()) {
    pushDiagnostic(
      ...
      "invalid_local_ref_type",
      fieldPath,
      "La ref locale attendue doit pointer vers un dossier.",
    );
  }
}
...
function containsForbiddenLocalRef(value: string): boolean {
  ...
  return FORBIDDEN_LOCAL_REF_PATTERNS.some((pattern) =>
    normalizedPath.includes(pattern.toLowerCase())
  );
}

function buildFieldPath(parentPath: string | undefined, key: string): string {
