...
exports.buildTicketBoardProjection = buildTicketBoardProjection;
const execution_attempt_1 = require("../../../contracts/src/execution-attempt/execution-attempt");
const ticket_board_projection_1 = require("../../../journal/src/projections/ticket-board-projection");
function buildTicketBoardProjection(mission, missionTickets, missionAttempts = [], missionEvents = []) {
    const ticketsById = new Map(missionTickets.map((ticket) => [ticket.id, ticket]));
    const attemptsByTicketId = groupAttemptsByTicketId(missionAttempts);
    const extensionUsageByTicketId = collectExtensionUsageByTicketId(missionEvents, mission.id);
    return {
        schemaVersion: 1,
        tickets: mission.ticketIds.flatMap((ticketId, planOrder) => {
            ...
            return [
                (0, ticket_board_projection_1.createTicketBoardEntry)(ticket, resolvePlanningMetadata(ticket, ticketsById, attempts, planOrder, extensionUsage)),
            ];
        }),
    };
}
function resolvePlanningMetadata(ticket, ticketsById, attempts, planOrder, extensionUsage) {
    ...
    const blockedByPendingDependencyIds = dependencyStatuses
        .filter((dependencyStatus) => dependencyStatus.status !== "done"
...
function resolveDependencyStatuses(ticket, ticketsById) {
...
function groupAttemptsByTicketId(missionAttempts) {
    const attemptsByTicketId = new Map();
...
function resolveActiveAttempt(attempts) {
    const activeAttempt = [...attempts]
        .reverse()
        .find((attempt) => execution_attempt_1.ACTIVE_EXECUTION_ATTEMPT_STATUSES.includes(attempt.status));
    return activeAttempt ? toAttemptSummary(activeAttempt) : null;
}
function resolveLastAttempt(attempts) {
...
function collectExtensionUsageByTicketId(missionEvents, missionId) {
    const usageByTicketId = new Map();
    for (const event of missionEvents) {
        ...
        if (event.type === "capability.invoked") {
            const capabilityId = readNestedString(event.payload, "capability", "capabilityId");
            if (capabilityId) {
        ...
        if (event.type === "skill_pack.used") {
            const packRef = readNestedString(event.payload, "skillPack", "packRef");
            if (packRef) {
...
function readNestedString(payload, parentKey, childKey) {
