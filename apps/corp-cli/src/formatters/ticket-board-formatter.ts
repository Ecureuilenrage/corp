import type {
  TicketBoardEntry,
  TicketStatusReasonCode,
} from "../../../../packages/journal/src/projections/ticket-board-projection";

export function formatTicketBoard(ticketBoardEntries: TicketBoardEntry[]): string[] {
  ...
  return [
    "Etat des tickets:",
    ...ticketBoardEntries.map((ticketBoardEntry, index) =>
      formatTicketBoardEntry(ticketBoardEntry, index)
    ),
  ];
}

function formatTicketBoardEntry(
  ticketBoardEntry: TicketBoardEntry,
  index: number,
): string {
  const parts = [
...
function formatDependencies(ticketBoardEntry: TicketBoardEntry): string {
...
function formatReason(ticketBoardEntry: TicketBoardEntry): string {
  ...
  if (
    ticketBoardEntry.statusReasonCode === "dependency_pending"
...
function formatAttemptSummary(ticketBoardEntry: TicketBoardEntry): string | null {
...
function formatExtensionUsage(ticketBoardEntry: TicketBoardEntry): string | null {
...
const REASON_LABELS: Record<TicketStatusReasonCode, string> = {
